...
"use strict";angular.module("core").config(["$stateProvider","$urlRouterProvider",function($stateProvider,$urlRouterProvider){$urlRouterProvider.otherwise(function($injector,$location){$injector.get("$state").transitionTo("not-found",null,{location:!1})}),$stateProvider.state("not-found",{url:"/not-found",templateUrl:"modules/core/client/views/404.client.view.html"})}]);
...
"use strict";angular.module("posts").factory("Posts",["$resource",function($resource){return $resource("api/posts/:postId",{postId:"@_id"},{update:{method:"PUT"}})}]).config(["twemojiProvider",function(twemojiProvider){twemojiProvider.setOptions({size:"16x16"})}]);
...
"use strict";angular.module("tweets").config(["$stateProvider",function($stateProvider){$stateProvider.state("tweets",{"abstract":!0,url:"/tweets",template:"<ui-view/>"}).state("tweets.list",{url:"",templateUrl:"modules/tweets/client/views/list-tweets.client.view.html"}).state("tweets.create",{url:"/create",templateUrl:"modules/tweets/client/views/create-tweet.client.view.html"}).state("tweets.view",{url:"/:tweetId",templateUrl:"modules/tweets/client/views/view-tweet.client.view.html"}).state("tweets.edit",{url:"/:tweetId/edit",templateUrl:"modules/tweets/client/views/edit-tweet.client.view.html"})}]);
...
"use strict";angular.module("users.admin.routes").config(["$stateProvider",function($stateProvider){$stateProvider.state("admin.users",{url:"/users",templateUrl:"modules/users/client/views/admin/user-list.client.view.html",controller:"UserListController"}).state("admin.user",{url:"/users/:userId",templateUrl:"modules/users/client/views/admin/user.client.view.html",controller:"UserController",resolve:{userResolve:["$stateParams","Admin",function($stateParams,Admin){return Admin.get({userId:$stateParams.userId})}]}}).state("admin.user-edit",{url:"/users/:userId/edit",templateUrl:"modules/users/client/views/admin/user-edit.client.view.html",controller:"UserController",resolve:{userResolve:["$stateParams","Admin",function($stateParams,Admin){return Admin.get({userId:$stateParams.userId})}]}})}]);
...
"use strict";angular.module("users").controller("AuthenticationController",["$scope","$state","$http","$location","$window","Authentication",function($scope,$state,$http,$location,$window,Authentication){$scope.authentication=Authentication,$scope.error=$location.search().err,$scope.authentication.user&&$location.path("/"),$scope.signup=function(){$http.post("/api/auth/signup",$scope.credentials).success(function(response){$scope.authentication.user=response,$state.go($state.previous.state.name||"home",$state.previous.params)}).error(function(response){$scope.error=response.message})},$scope.signin=function(){$http.post("/api/auth/signin",$scope.credentials).success(function(response){$scope.authentication.user=response,$state.go($state.previous.state.name||"home",$state.previous.params)}).error(function(response){$scope.error=response.message})},$scope.callOauthProvider=function(url){$state.previous&&$state.previous.href&&(url+="?redirect_to="+encodeURIComponent($state.previous.href)),$window.location.href=url}}]);
...
"use strict";angular.module("videos").config(["$stateProvider",function($stateProvider){$stateProvider.state("videos",{"abstract":!0,url:"/videos",template:"<ui-view/>"}).state("videos.slick",{url:"",templateUrl:"modules/videos/client/views/slick-videos.client.view.html"}).state("videos.list",{url:"/list",templateUrl:"modules/videos/client/views/list-videos.client.view.html"}).state("videos.create",{url:"/create",templateUrl:"modules/videos/client/views/create-video.client.view.html"}).state("videos.view",{url:"/:videoId",templateUrl:"modules/videos/client/views/view-video.client.view.html"}).state("videos.edit",{url:"/:videoId/edit",templateUrl:"modules/videos/client/views/edit-video.client.view.html"})}]);
