...
function makeMongooseReq (req, groupeBy) {
    ...
    var reqMongo;
    var agg = [{ $match : {'created_at': {'$gte': dateStart, '$lt': dateEnd}}}];
    ...
    if (req.query.to !== undefined && req.query.to) {
        ...
        agg.push({$match : {to : {$in : req.query.to }}});
    }


    if (groupeBy !== undefined) {
        agg.push({ $match : { 'to' : {'$ne' : null}}});
        agg.push({$group: {
            _id: '$' + groupeBy,
            from_user_profile_image_url : { $push: '$user.profile_image_url' },
            from : { $push: '$from' },
            to : { $push: '$to' },
            total: {$sum: 1}
        }});
        agg.push({$sort : {total : -1}});
    } else {
    ...
    if (req.query.start !== undefined) {
        agg.push({$skip : parseInt(req.query.start.toString())});
    }
    //Param limit
    if (req.query.limit !== undefined) {
        agg.push({$limit : parseInt(req.query.limit.toString())});
    }
 ...
 * Tweet middleware
 */
exports.tweetByID = function(req, res, next, id) { Tweet.findById(id).populate('displayName').exec(function(err, tweet) {
...
exports.getInformations = function(req, res) {
    ...
    makeMongooseReq(req, 'to')
        ...
        function (tweets) {
            ...
            var user = _.find(tweets, { '_id': userName });
